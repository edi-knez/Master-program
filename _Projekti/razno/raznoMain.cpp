...
void ispisi_datoteku_u_konzolu( std::fstream& datoteka, const std::string& imeDatoteke )
{
	...
	{
		for( size_t counterChars = 0, counterForceBreak = 0, endOfWordPos = 0; counterChars < 60 * 127; ++counterChars, ++counterForceBreak )
		{
			if( datoteka.eof() )
			{
			...
			else if( c == '.' && counterForceBreak > 100 )
			{
			  // nemoj duplicirat novu liniju
				if( datoteka.peek() == '\n' ) datoteka.seekg( 1, std::ios::cur );
				buf += '\n';
				++counterChars;
				counterForceBreak = 0;
			}
			else if( c == ',' && counterForceBreak > 90 )
			{
				buf += '\n';
				counterForceBreak = 0;
				endOfWordPos = 0;
			}
			else if( c == ' ' )		endOfWordPos = counterForceBreak;

			//  or at the end of a word if a sentence is too long.
			else if( counterForceBreak == 127 )
			{
				++counterChars;
				for( size_t i = counterChars; i > counterChars + 1 - ( 127 - endOfWordPos ); --i )	// shiftaj sve znakove za jedno mjesto
				{
					buf[i - 1] = buf[i];
				}
				if( endOfWordPos > 0 )
				{
					buf[1 + counterChars - ( 127 - endOfWordPos )] = '\n';
					endOfWordPos = 0;
				}
				else
				{
