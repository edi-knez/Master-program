...
namespace Master
{
	extern 	std::unordered_map<std::string, std::string> konfiguracijskePostavke;
	...
	void popuniCijeliPopisFunkcija( const nlohmann::json& jsonData, std::vector<std::unordered_map<std::string, std::unordered_map<std::string, std::pair<Zadatak, size_t>>>>& spremnikFunkcijaPoCjelinama, bool isExecutionProcess );
	void popuniCijeliPopisFunkcija( const nlohmann::json& jsonData, std::vector<std::unordered_map<std::string, std::unordered_map<std::string, std::pair<Zadatak, size_t>>>>& spremnikFunkcijaPoCjelinama );

	namespace _INTERNAL
	{

		void popuniPopisFunkcijaZa( const nlohmann::json& jsonData, std::vector<std::unordered_map<std::string, std::unordered_map<std::string, std::pair<Zadatak, size_t>>>>& spremnikFunkcijaPoCjelinama, const size_t projIdx, std::ostream& funcDat, bool isExecutionProcess );
		void popuniPopisFunkcijaZa( const nlohmann::json& jsonData, std::vector<std::unordered_map<std::string, std::unordered_map<std::string, std::pair<Zadatak, size_t>>>>& spremnikFunkcijaPoCjelinama, const size_t projIdx );
...
void Master::_INTERNAL::utvrdiOKojemPovratnomTipuSeRadi( std::string& funcRetType, const std::optional<std::unordered_map<std::string, size_t>>& ext2, const std::string_view deklaracija, const std::string_view kod )
{
	...
	if( ext2.has_value() )
	{
		...
		if( bool isTrailingReturnType = it != ext2->end() )
		{
			...
			startIt = std::find_if( startIt, deklaracija.end(), []( char c ){ return !isspace( c ); } );
			auto endIt = std::find_if( startIt, deklaracija.end(), []( char c ){ return isspace( c ) || c == '\n'; } );
...
[[nodiscard]] std::pair<std::string, std::optional<std::unordered_map<std::string, bool/*unused*/>>> Master::_INTERNAL::getFuncReturnType( const std::string& deklaracija, size_t& offset )
{
	...
	std::string retVal1;
	std::unordered_map<std::string, bool/*unused*/> pronadeneKvalifikacije;
	do	{
		...
		if( isspace( *curIt ) )	// nalazi se na zadnjem znaku kvalifikacije
		{
			...
			{
				pronadeneKvalifikacije.insert( { trenutacnaRijec, false } );
			}
	...
	std::optional<std::unordered_map<std::string, bool/*unused*/>> retVal2( std::move( pronadeneKvalifikacije ) );
...
[[nodiscard]] std::pair<std::string, std::optional<std::unordered_map<std::string, size_t/*unused*/>>> Master::_INTERNAL::getFuncArguments( const std::string& deklaracija, size_t& offset )
{
	...
	auto kvalifikacijeBegin = std::find_if_not( krajListeArgumenata + 1, deklaracija.end(), []( char c ){ return isspace( c ); } );
	auto kvalifikacijeEnd = std::find_if( kvalifikacijeBegin, deklaracija.end(), []( char c ){ return isspace( c ); } );
	std::string kvalifikacije( kvalifikacijeBegin, kvalifikacijeEnd );
	if( kvalifikacije.empty() )	retVal2 = {};
	if( kljucneRijeci.contains( kvalifikacije ) && kvalifikacije == "->" )
	{
		std::unordered_map<std::string, size_t/*unused*/> t;
		t.insert( std::pair( kvalifikacije, size_t( kvalifikacijeEnd - deklaracija.begin() ) ) );
		retVal2.emplace( t );
	}
	else if( kljucneRijeci.contains( kvalifikacije ) && kljucneRijeci[kvalifikacije] == false )	retVal2->emplace( kvalifikacije, false );
	return { retVal1, retVal2 };
}
...
void Master::popuniCijeliPopisFunkcija( const nlohmann::json& jsonData, std::vector<std::unordered_map<std::string, std::unordered_map<std::string, std::pair<Zadatak, size_t>>>>& spremnikFunkcijaPoCjelinama )
{
	size_t idx = 0;
	auto curProjIt = jsonData.begin();
	while( curProjIt != jsonData.end() )
	{
		spremnikFunkcijaPoCjelinama.push_back( {} );
		Master::_INTERNAL::popuniPopisFunkcijaZa( curProjIt.value(), spremnikFunkcijaPoCjelinama, idx );
		++curProjIt;
		++idx;
	}
}

void Master::popuniCijeliPopisFunkcija( const nlohmann::json& jsonData, std::vector<std::unordered_map<std::string, std::unordered_map<std::string, std::pair<Zadatak, size_t>>>>& spremnikFunkcijaPoCjelinama, bool isExecutionProcess )
{
	...
	size_t idx = 0;
	for( const auto& thisProj : spremnikFunkcijaPoCjelinama )
	{
		Master::_INTERNAL::popuniPopisFunkcijaZa( jsonData[idx], spremnikFunkcijaPoCjelinama, idx, datotekaZaSpremanjeFunkcija, isExecutionProcess );
		++idx;
	}
...
void Master::_INTERNAL::popuniPopisFunkcijaZa( const nlohmann::json& jsonData, std::vector<std::unordered_map<std::string, std::unordered_map<std::string, std::pair<Zadatak, size_t>>>>& spremnikFunkcijaPoCjelinama, const size_t projIdx, std::ostream& funcDat, bool isExecutionProcess )
{
...
///	std::clog << "\n\n" << cjelEntryObj << "\n\n";
	...
	for( const auto& cjelina : cjelineData.second )
	{
		...
		for( auto cj = cjelina.begin(); cj != cjelina.end(); ++cj )
		{
			...
			spremnikFunkcijaPoCjelinama[projIdx].insert( { brojCjeline, std::move( CjelinaX ) } );
			auto& iter = spremnikFunkcijaPoCjelinama[projIdx].find( brojCjeline )->second;
			for( const auto& zadatak : cjelEntryObj["Zadaci"] )
			{
				if( zadatak.size() == 0 )	break;
				...
				auto startIt = std::find_if( endIt, deklaracija.rend(), []( char c ){ return c == ':' || isspace( c ); } );
				...
				if( isExecutionProcess == false )
				{
					if( true )	funcDat << '\t' << "DODAJ_FUNKCIJU( " << projIdx << ", " << brojCjeline << ", " << imeZadatka << " );\n";
...
void Master::_INTERNAL::popuniPopisFunkcijaZa( const nlohmann::json& jsonData, std::vector<std::unordered_map<std::string, std::unordered_map<std::string, std::pair<Zadatak, size_t>>>>& spremnikFunkcijaPoCjelinama, const size_t projIdx )
{
	json::object_t cjelEntryObj;
	if( jsonData.begin()->is_object() )
		cjelEntryObj = jsonData.begin().value();
	else
		return;
///	std::clog << "\n\n" << cjelEntryObj << "\n\n";
	const auto& extractedCjelinaEntryValue = cjelEntryObj.extract( "Broj cjeline" );
	// nazivi projekata su spremljeni u std::map pod. strukturu u json objektu, znaci sortirani su
	std::pair<std::string, json::array_t> cjelineData;
	if( extractedCjelinaEntryValue._Getptr() )
		cjelineData = extractedCjelinaEntryValue._Getptr()->_Myval;
	else
		return;
	for( const auto& cjelina : cjelineData.second )
